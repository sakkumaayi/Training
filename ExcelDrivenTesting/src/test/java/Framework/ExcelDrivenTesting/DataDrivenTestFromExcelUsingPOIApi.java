...
//to read data from xcel file
//->get access to excel 
//then go to the desired sheet
//then go to first row inthe desired sheet and traverse through all the cells
//then compare cells data with required column name and get column index(number)
//iterate through all the rows
public class DataDrivenTestFromExcelUsingPOIApi {
	public ArrayList<String> getDataFromExcel(String testcase) throws IOException {
		...
		//got access to excel file using XSSFWorkbook
		int sheetcount=workbook.getNumberOfSheets();//number of sheets
		for (int i=0;i<sheetcount;i++) {
			if(workbook.getSheetName(i).equalsIgnoreCase("testdata")){//get all sheet names and compare with desired sheet name 
